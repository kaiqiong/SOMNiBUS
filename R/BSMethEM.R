...
BSMethEM <- function(data, n.k, p0 = 0.003, p1 = 0.9, Quasi = TRUE, epsilon = 10^(-6),
  ...
  colnames(data)[match(c("Meth_Counts", "Total_Counts", "Position"), colnames(data))] <- c(
    "Y",
    "X", "Posit"
  )
  ...
  if (Quasi & scale <= 0) {
    ...
    # Note: the estimator implemented in the mgcv calculated my_s with an additional
    # multiplier sqrt(data$X) But from the paper there shouldn't be this one phi_p =
    # sum( p_res^2)/(length(data$Y) - sum(edf.out))

    # Feb 21, 2020 use edf1 to calculate the pearson's dispersion estimate not the
    # edf; because I will use the asympototic chi-square dist. of phi.est March 3,
    # 2020, the dispersion parameter estimated in gam use the edf.out instead of
    # edf1.out
  ...
  if (p0 == 0 & p1 == 1) {
    out <- list(
      pi.ij = gam.int$fitted.values, par = gam.int$coefficients, lambda = gam.int$sp,
      edf1 = gam.int$edf1, pearson_res = p_res, deviance_res = d_res, edf = gam.int$edf,
      phi_fletcher = phi_fletcher, GamObj = gam.int
    )
    new.par <- out$par
    ...
    out <- BSMethEMUpdate(data, old.pi.ij,
      ...
      method = method, Z = Z, my.covar.fm = my.covar.fm, Quasi = Quasi, scale = phi_fletcher
    )
    new.par <- out$par
    new.lambda <- out$lambda
    new.pi.ij <- out$pi.ij
    ...
    i <- 1
    Est.points <- rbind(c(old.par, lambda, phi_fletcher), c(
      new.par, new.lambda,
      new.phi
    ))
    # Do the iteration The stopping criterion is that estimator a and b are close
    # enough I exclude the criterio that lambda-a and lambda-b are close
    ...
    while (sqrt(sum((new.pi.ij - old.pi.ij)^2)) > epsilon & i < maxStep) {
      ...
      out <- BSMethEMUpdate(data, old.pi.ij,
        p0 = p0, p1 = p1, binom.link = binom.link,
  ...
  # Effective degrees of freedom: edf1 -- good for chisquare test and p value
  # calculation tr(2A - A^2)
  edf1.out <- out$edf1
  ...
  #-------------------------------------------------------------------------
  # from the variance-covariance of alpha, to report 1. var(beta_p(t)) 2. Report
  # the chi-square statistic for each covariate 3. Report the p-value for each
  # covariate
  #-------------------------------------------------------------------------
  ...
  # Simply check to see tryCatch({
  #----------------------------------------------------------------
  # calculate var_cov (for alpha & beta) from the Hessian matrix
  #----------------------------------------------------------------

  H <- Hessian(
    w_ij = pred.pi * (1 - pred.pi) / phi_fletcher, new.par, new.lambda,
    X, Y, my.design.matrix, GamObj, Z, pred.pi, p0, p1, disp_est = phi_fletcher,
    RanEff = RanEff, N = N
  )
  ...
  # A more efficient way to extract design matrix. use a random sample of rows of
  ...
  # my.design.matrix V: estimated variance matrix on entry `rank' should be an edf
  # estimate 0. Default using the fractionally truncated pinv.  1. Round down to k
  # if k<= rank < k+0.05, otherwise up.  res.df is residual dof used to estimate
  # scale. <=0 implies fixed scale.

  s.table <- BSMethEM_summary(
    GamObj, var.cov.alpha, new.par, edf.out, edf1.out,
    X_d, resi_df, Quasi, scale, RanEff, re.test, Z
  )
  s.table.REML.scale <- BSMethEM_summary(
    GamObj, var.cov.alpha / phi_fletcher * phi_reml,
  ...
  return(out = list(
    est = new.par, lambda = new.lambda, est.pi = new.pi.ij, Beta.out = Beta.out,
    phi_fletcher = phi_fletcher, phi_reml = phi_reml, reg.out = s.table, reg.out.reml.scale = s.table.REML.scale,
    cov1 = var.cov.alpha, reg.out.gam = reg.out.gam, SE.out = SE.out, SE.out.REML.scale = SE.out.REML.scael,
    uni.pos = SE.pos, ncovs = ncol(Z) + 1, ite.points = Est.points, sigma00 = sigma00
  ))
}
...
#' @importFrom Matrix bdiag
Hessian <- function(w_ij, new.par, new.lambda, X, Y, my.design.matrix, gam.int, Z,
                    pred.pi, p0, p1, disp_est, RanEff, N) {
...
#' # Lorem ipsum dolor sit amet
#' @import mgcv
BSMethEM_summary <- function(GamObj, var.cov.alpha, new.par, edf.out, edf1.out, X_d,
                             resi_df, Quasi, scale, RanEff, re.test, Z) {
  ii <- 0
  ...
  for (i in seq_len(m)) {
    ...
    if (!fx && GamObj$smooth[[i]]$null.space.dim == 0 && !is.null(GamObj$R)) {
      ...
      ## and accounting for all random effects in model Inverted Nychka interval
      ## statistics
    } else {
      if (Quasi) {
        rdf <- resi_df
    ...
    if (!Quasi || scale >= 0) {
      ...
      dimnames(s.table) <- list(names(chi.sq), c(
        "edf", "Ref.df", "Chi.sq",
        "p-value"
      ))
    } else {
