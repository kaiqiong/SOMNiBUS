...
```{r, eval =TRUE}
# devtools::install_git("https://github.com/kaiqiong/SOMNiBUS.git")
library(SOMNiBUS)
```

Make sure to run `install.packages("pacman")` if you have not installed the `pacman` package.
...
The smooth covariate estimation and the region-wise test steps are wrapped into a function `BSMethEM`. See `help(BSMethEM)` for more details. We can use the following code to run the analysis with both covariates `T_cell` and `RA`.

```{r}
out <- BSMethEM(data = RAdat.f, n.k = rep(5, 3), p0 = 0.003, p1 = 0.9, Quasi = FALSE, RanEff = FALSE)
```
...
plot_BSMethEM(out, same.range = TRUE)
```
...
```{r}
time0 <- Sys.time()
out_quasi <- BSMethEM(data = RAdat.f, n.k = rep(5, 3), p0 = 0.003, p1 = 0.9, Quasi = TRUE, RanEff = FALSE)
...
time0 <- Sys.time()
out_RA2 <- BSMethEM(data = RAdat2, n.k = c(5, rep(max(3, round(nCpG / 20)), 9)), p0 = 0.003, p1 = 0.9, Quasi = TRUE, RanEff = FALSE)
print(Sys.time() - time0)
```
```{r}
out_RA2$reg.out
...
```{r}
plot_BSMethEM(out_RA2, mfrow = c(2, 5))
```
...
```{r}
time0 <- Sys.time()
out_RA2_noError <- BSMethEM(data = RAdat2, n.k = c(5, rep(max(3, round(nCpG / 20)), 9)), p0 = 0, p1 = 1, Quasi = TRUE, RanEff = FALSE)
print(Sys.time() - time0)
```
