...
test_that("all parameters values get closer along EM optimization procedure", {
  ...
  for (i in 1:(length(cols) - 1)) {
    diff_first_last <- abs(out_BSMethEM$ite.points[1, i] - out_BSMethEM$ite.points[
      n_iter,
      i
    ])
    diff_2_last <- abs(out_BSMethEM$ite.points[n_iter, i] - out_BSMethEM$ite.points[n_iter -
      1, i])
...
test_that("the number of unique positions output by BSMethEM corresponds to the number of unique positions found in the data", {
  expect_equal(length(out_BSMethEM$uni.pos), length(unique(RAdat.f$Position)))
})

test_that("the genomic positions are well ordered in BSMethEM output (ascending order)", {
  a_list_bool <- c()
  for (i in 1:(length(out_BSMethEM$uni.pos) - 1)) {
    a_list_bool <- c(a_list_bool, out_BSMethEM$uni.pos[i] < out_BSMethEM$uni.pos[i +
      1])
  }
